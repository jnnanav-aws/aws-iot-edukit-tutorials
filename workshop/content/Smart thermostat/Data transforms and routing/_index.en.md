...
Why use sound level and simple threshold banding to classify room occupancy instead of a motion sensor? In this case, a microphone is the sensor that is available for use. When designing IoT solutions, you will not always have the budget for the best possible input data. This approach strikes a balance of frugality and achieving the use case. We acknowledge that this would not work for every use case; such as, a team meeting where the participants use sign language, or for team meetings that start with a silent document review.
